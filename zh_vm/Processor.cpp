...
Status Processor::eval(Word *program, size_t size)
{
	...
	while (continue_eval)
	{
		...
		case 16:
			...
			hardware.set_stack(hardware.get_register(ins->op0), hardware.get_register(ins->op1));
			break;
		case 17:
			printf(ADDR_STR "pop %x %x\n", pc_offset, ins->op0, ins->op1);
			hardware.set_register(hardware.get_register(ins->op0), hardware.get_stack(ins->op1));
			break;
		case 18:
